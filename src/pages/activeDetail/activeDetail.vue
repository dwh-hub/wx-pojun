...
export default {
  ...
  onLoad(options) {
    ...
    this.storeList.forEach((e) => {
      this.actionStore.push(e.storeName)
    })
  },
  onUnload() {
    this.activeDetail = {}
  ...
  methods: {
    ...
    showPopup() {
      ...
      wx.showActionSheet({
        itemList: this.actionStore,
        itemColor: window.color,
        success(res) {
          console.log('tapIndex:'+res.tapIndex)
          that.storeIndex = res.tapIndex
          that.showEntryBox = true;
        }
      })
    },
    confirmSignUp() {
      ...
      HttpRequest({
        ...
        data: {
          ...
          storeId: that.storeList[that.storeIndex].storeId,	// 报名门店
