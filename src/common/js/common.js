...
window.domain = "https://www.pojun-tech.cn" // 服务器域名
window.api = window.DEBUGGING ? "http://192.168.1.212" : window.domain
      ...
      success(res) {
        if (res.code) {
          wx.request({
            ...
            data: {
              code: res.code,
              companyId: window.isPublic ? '55' : wx.getStorageSync("companyId")
            },
            success(data) {
