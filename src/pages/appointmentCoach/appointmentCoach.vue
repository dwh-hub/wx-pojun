  ...
  <div class="appointment-coach" :class="{'isPhoneX-wrap':isPhoneX}">
    ...
    <van-popup
      ...
      <div class="date-group">
        ...
        <select-time ref="selectTime" @selectDate="selectDate" :dayDate="curDate" :openStoreTime="openTimeStart" closetoreTime:="openTimeEnd" :periodTime="todayPeriodTime"></select-time>
    ...
    <van-popup class="cards-pop" :show="isCardPopup" position="bottom" @close="isCardPopup = false">
      ...
      <div class="cards-list">
        <div
          ...
          :key="index"
        >
          <div class="cover" :style="{'background-image': item.color}">
...
export default {
  ...
  methods: {
    ...
    selectDate(timeObj) {
      ...
      this.curTime = timeObj.startTime
      this.curEndTime = timeObj.endTime
      // this.timeCellText = this.confirmDate
      this.timeCellText = timeObj.confirmDate.replace(/（/g, "").replace(/）/g, "");
      if (!this.cardClassId) {
        // this.isCardPopup = true;
        this.showCardPopop();
      }
        ...
        success(res) {
          if (res.data.code == 200) {
            ...
            _list = _list.filter(e => {
              ...
              return (
                e.teachCardType == 3 ||
                (e.canTeachCard == 1 && e.teachCardType == 2)
              );
            });

            that.cardList = _list.map(e => {
              if (e.authorityUnit == '元') {
                e.backImg = window.api + "/assets/img/yuanka.png";
                e.color = 'linear-gradient(to left top, #37d8a9, #0dbbc9)'
              }
              if (e.authorityUnit == '天') {
                e.backImg = window.api + "/assets/img/qixianka.png";
                e.color = 'linear-gradient(to left top, #ffc66c, #ff7f44)'
              }
              if (e.authorityUnit == '次') {
                e.backImg = window.api + "/assets/img/cardImg.png";
                e.color = 'linear-gradient(to left top, #87c6ff, #159bff)'
              }
              return e
            });
